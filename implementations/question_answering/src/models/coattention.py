...
class Encoder(nn.Module):
    ...
    def forward(self, seq, mask):
        ...
        sentinel_zero = torch.zeros(b, 1, self.hidden_size)
        if use_cuda:
...
class FusionBiLSTM(nn.Module):
    def __init__(self, hidden_size, dropout_ratio):
        ...
        self.fusion_bilstm = nn.LSTM(3 * hidden_size, hidden_size, 1, batch_first=True,
...
class MaxOutHighway(nn.Module):
    def __init__(self, hidden_size, maxout_pool_size, dropout):
        ...
        self.r = nn.Linear(5 * hidden_size, hidden_size, bias=False)
        #self.dropout_r = nn.Dropout(p=dropout_ratio)

        self.m_t_1_mxp = nn.Linear(3 * hidden_size, hidden_size * maxout_pool_size)
        #self.dropout_m_t_1 = nn.Dropout(p=dropout_ratio)

        self.m_t_2_mxp = nn.Linear(hidden_size, hidden_size * maxout_pool_size)
        #self.dropout_m_t_2 = nn.Dropout(p=dropout_ratio)

        self.m_t_12_mxp = nn.Linear(2 * hidden_size, maxout_pool_size)
    ...
    def forward(self, h_i, U, curr_mask, idx_i_1, u_cat, mask_mult, target=None):
        ...
        r = F.tanh(self.r(torch.cat((h_i.view(-1, self.hidden_size), u_cat), 1)))  # b x 5l => b x l
        #r = self.dropout_r(r)

        r_expanded = r.unsqueeze(1).expand(b, m, self.hidden_size).contiguous()  # b x m x l

        m_t_1_in = torch.cat((U, r_expanded), 2).view(-1, 3*self.hidden_size)  # b*m x 3l

        m_t_1 = self.m_t_1_mxp(m_t_1_in)  # b*m x p*l
        #m_t_1 = self.dropout_m_t_1(m_t_1)
        m_t_1, _ = m_t_1.view(-1, self.hidden_size, self.maxout_pool_size).max(2) # b*m x l

        m_t_2 = self.m_t_2_mxp(m_t_1)  # b*m x l*p
        #m_t_2 = self.dropout_m_t_2(m_t_2)
        m_t_2, _ = m_t_2.view(-1, self.hidden_size, self.maxout_pool_size).max(2)  # b*m x l

        alpha_in = torch.cat((m_t_1, m_t_2), 1)  # b * m * 2
        alpha = self.m_t_12_mxp(alpha_in)  # b * m * p
...
class CoAttention(BaseModel):
    def __init__(self, params: AttrDict, dataset):
        ...
        self.encoder = Encoder(dataset.vocab_size, cfg.embedding_dim, cfg.hidden_size, cfg.dropout)
    ...
    def get_mask_from_seq_len(self, seq_lens):
        ...
        return maybe_cuda(torch.LongTensor(mask))

    def forward(self, batch: Batch):
        q_seq, d_seq = batch.X
        q_len, d_len = batch.X_len
        ...
        # project q
        Q = F.tanh(self.q_proj(Q.view(-1, self.hidden_size))).view(Q.size())  # B x n + 1 x l
